...
export const DashboardPage = () => {
    ...
    const pageInfo = usePageInfo('dashboard');
    ...
    const loadStats = async () => {
        setLoading(true);
        try {
            // Use allSettled so each API call is independent — one failure doesn't block the rest
            const [knowledgeRes, escalationRes, broadcastRes, summaryRes] = await Promise.allSettled([
                knowledgeApi.getStats(),
                escalationApi.getStats(),
                broadcastApi.getStats(),
                analyticsApi.analyzeDay(selectedDate)
            ]);

            setStats({
                knowledge: knowledgeRes.status === 'fulfilled'
                    ? knowledgeRes.value.data.data
                    : { totalFaqs: 0, approvedFaqs: 0, pendingFaqs: 0 },
                escalation: escalationRes.status === 'fulfilled'
                    ? escalationRes.value.data.data
                    : { total: 0, pending: 0, resolved: 0 },
                broadcast: broadcastRes.status === 'fulfilled'
                    ? broadcastRes.value.data.data
                    : { totalCampaigns: 0, totalMessagesSent: 0 },
            });
                    ...
                    <>
                        ...
                        <div className="mb-8">
                            <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-4">
                                ...
                                <input
                                    ...
                                    className="px-3 py-1.5 border dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 w-full sm:w-auto"
                                />
                            </div>
                            {dailySummary ? (
                                ...
                                <div className="bg-white dark:bg-gray-800 rounded-lg border dark:border-gray-700 p-6 text-center">
                                    ...
                                    <p className="text-gray-500 dark:text-gray-400 text-sm">
                                        No hay datos de asistencia para {new Date(selectedDate + 'T12:00:00').toLocaleDateString('es-DO', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
                                    </p>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="bg-white dark:bg-gray-800 rounded-lg border dark:border-gray-700 p-4 sm:p-6 mb-6 transition-colors">
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
                                ...
                                <QuickAction
                                    title="Recordatorios"
                                    description="Clases del día"
                                    icon={Bell}
                                    color="orange"
                                    onClick={() => navigate('/attendance?tab=rules')}
                                />
                                <QuickAction
                                    title="Contactos"
